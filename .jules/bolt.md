...
## 2024-11-23 - [Use dataset for HTML attributes in string templating]
**Learning:** When using JavaScript template literals to construct HTML elements, injecting dynamic strings containing quotes (like filenames) into event handlers (e.g., `onclick="doSomething('${escapeHtml(filename)}')"` can lead to Uncaught SyntaxErrors. This happens because the browser decodes `&#039;` back to a single quote before the JavaScript executes, breaking the literal string syntax.
**Action:** Instead of passing dynamic strings directly to inline JS event handlers, use `data-` attributes to safely store the value (e.g., `data-filename="${escapeHtml(filename)}"`) and access it within the inline handler via `this.dataset.filename` (e.g., `onclick="doSomething(this.dataset.filename)"`).
